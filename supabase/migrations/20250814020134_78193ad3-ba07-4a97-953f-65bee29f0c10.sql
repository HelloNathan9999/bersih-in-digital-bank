-- Fix Authentication Security Vulnerabilities (Corrected Version)
...
-- 5. Create function to securely create auth sessions
...
BEGIN
  ...
  VALUES (
    ...
    jsonb_build_object('session_id', new_session_id, 'device_id', p_device_id),
    p_ip_address, now()
  );
...
-- 7. Create secure session deactivation function
...
BEGIN
  ...
  -- Only allow user to deactivate their own session
  ...
  VALUES (
    session_user_id, 'session_deactivated', 
...
-- 8. Update auth_sessions RLS policies for enhanced security
...
-- 9. Create function to validate session security
...
BEGIN
  ...
  -- Check for suspicious changes (IP mismatch)
...
-- 10. Create automated security maintenance function
